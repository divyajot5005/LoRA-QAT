...
MAIN_LOG="$LOG_ROOT/watchdog_box2.log"
...
while true; do
  start_if_missing "queue_l40_lora_box2.sh" \
    "nohup bash $REPO_DIR/queue_l40_lora_box2.sh > $LOG_ROOT/launcher_box2.log 2>&1 < /dev/null &" \
    "queue_l40_lora_box2"
  sleep "$SLEEP_SECONDS"
done
